...
export const Dashboard = () => {
  ...
  const fetchTournaments = async () => {
    try {
      const { data, error } = await fetchDashboardTournaments();
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className={`text-xs px-2.5 py-1 rounded-full flex items-center gap-1.5 ${
                    tournament.status === 'active' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                    ...
                    {(() => {
                      ...
                      const format = (config as TournamentConfig)?.original_format || tournament.format;
                      
                      return format === 'groups' ? 'Fase de Grupos + Playoffs' :
                             format === 'single_elim' ? 'Eliminación Directa' : 
                             format === 'double_elim' ? 'Doble Eliminación' :
                             format === 'swiss' ? 'Suizo' : format;
                    })()}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-4 text-sm text-text-muted mt-6 pt-4 border-t border-white/5">
                  <div className="flex items-center gap-1.5">
                    <Users size={16} />
                    <span>{(tournament.config as TournamentConfig)?.participants_count || 0} Participantes</span>
                  </div>
                  <div className="flex items-center gap-1.5 ml-auto">
                    <span className="text-xs group-hover:translate-x-1 transition-transform">Gestionar</span>
