...
interface BracketMatch extends Omit<Match, 'id' | 'created_at'> {
  id?: string; // Optional for new matches before insertion
}
...
export const generateGroupStageMatches = (
  ...
  const participantsPerGroup = Math.ceil(count / groupCount);
