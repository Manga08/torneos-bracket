          ...
          <div className="grid gap-3">
            {matchesByRound[round].map(match => {
              ...
              return (
                ...
                >
                  {/* Status Indicator */}
                  <div className={`match-list-indicator ${isCompleted ? 'bg-success' : 'bg-surface-highlight'}`} />
                  
                  <div className="flex justify-between items-center gap-4 relative z-10">
                    {/* Participant A */}
                    <div className="flex-1 flex items-center gap-3 overflow-hidden">
                      <div className={`font-medium truncate font-display tracking-wide match-participant-name ${match.winner_id === match.participant_a_id ? 'match-winner' : ''}`}>
                        {pA?.name || <span className="text-text-muted italic">TBD</span>}
                      </div>
                      {isCompleted && <span className="font-mono font-bold text-lg match-score">{match.score_a}</span>}
                    </div>
                    ...
                    <div className="flex-1 flex items-center justify-end gap-3 overflow-hidden">
                      {isCompleted && <span className="font-mono font-bold text-lg match-score">{match.score_b}</span>}
                      <div className={`font-medium truncate text-right font-display tracking-wide match-participant-name ${match.winner_id === match.participant_b_id ? 'match-winner' : ''}`}>
                  ...
                  <div className="mt-2 flex justify-between items-center text-xs match-meta">
                    ...
                    {isCompleted ? (
                      <span className="flex items-center gap-1 match-status-completed"><CheckCircle size={10} /> Finalizado</span>
                    ) : (
                      <span className="flex items-center gap-1 match-status-pending"><Clock size={10} /> Pendiente</span>
                    )}
                  </div>
                </div>
              );
