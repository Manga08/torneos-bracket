...
export const BracketView = ({ 
  ...
  useEffect(() => {
    if (isDraft) {
      ...
      if (format === 'double_elim') {
        // @ts-expect-error - Types mismatch for draft
        virtualMatches = generateDoubleEliminationMatches(tournamentId, participants) as unknown as Match[];
      } else if (format === 'swiss') {
        // @ts-expect-error - Types mismatch for draft
        virtualMatches = generateSwissMatches(tournamentId, participants) as unknown as Match[];
      } else if (format === 'groups') {
        // @ts-expect-error - Types mismatch for draft
        virtualMatches = generateGroupStageMatches(tournamentId, participants) as unknown as Match[];
      } else {
        // @ts-expect-error - Types mismatch for draft
        virtualMatches = generateSingleEliminationMatches(tournamentId, participants, hasThirdPlace) as unknown as Match[];
      }
  ...
  const renderBracketSection = (stage: string, title: string) => {
    ...
    let stageRounds = matchesByStage[stage] || {};
      ...
      {!isDraft && (format === 'groups' || format === 'swiss') && (
        <div className="mb-12 space-y-8">
           ...
           {Object.keys(matchesByStage).filter(s => s.startsWith('Group')).sort().map(stage => {
             const stageMatches = Object.values(matchesByStage[stage]).flat();
